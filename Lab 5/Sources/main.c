...
void TowerInit()
{
  ...
  error = OS_ThreadCreate(GetTimePacket, NULL, &GetTimePacketThreadStack[THREAD_STACK_SIZE - 1], PRIORITY_TIME_THREAD);
...
bool AnalogData(const uint8_t channelNb)
{
  ...
  //TODO: Write a function that does 1 channel and do it for 4 channels
...
void PacketThread (void* pData)
{
  ...
  for (;;)
  {
    //TODO: Make Packet Get a blocking function
    if (Packet_Get(pData))
    {
      // OS_EnableInterrupts();
...
/*lint -restore Enable MISRA rule (6.3) checking. */
{
  ...
  //Create the Init Thread for the Operating System
